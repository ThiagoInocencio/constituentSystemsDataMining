...
dataImputation = function(temp) {
  if(any(is.na(temp))) {
    ...
    imp = impute(temp, target =character(0), classes = list(integer = imputeMean(), factor = imputeMode(), character = imputeMode())
    )
    temp = imp$data
  }
  return(temp)
...
write.csv(newData,'R/datasets/data_set_base_B.csv', row.names = FALSE)
